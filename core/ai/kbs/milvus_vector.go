...
func NewMilvusVectorStore(
	...
	if err != nil {
		logs.Warnf("failed to get embedding dimension, using default dim %d: %v", defaultDim, err)
	} else if len(testVectors) > 0 && len(testVectors[0]) > 0 {
		dim = len(testVectors[0])
		logs.Infof("detected embedding dimension: %d", dim)
	}

	//先创建collection
	err = ensureMilvusCollection(ctx, c, collectionName, dim)
	if err != nil {
		return nil, err
	}
...
func ensureMilvusCollection(ctx context.Context, client client.Client, collectionName string, dim int) error {
	//先判断collection是否存在
