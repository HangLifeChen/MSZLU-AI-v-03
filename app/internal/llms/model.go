...
func (r *models) updateProviderConfig(ctx context.Context, config *model.ProviderConfig) error {
	return r.db.WithContext(ctx).Save(config).Error
}

func (r *models) getProviderConfigByID(ctx context.Context, id uuid.UUID) (*model.ProviderConfig, error) {
	var pc model.ProviderConfig
	err := r.db.WithContext(ctx).Where("id = ?", id).First(&pc).Error
	if gorms.IsRecordNotFoundError(err) {
		return nil, nil
	}
	return &pc, err
}
func (r *models) deleteProviderConfig(ctx context.Context, id uuid.UUID) error {
	return r.db.WithContext(ctx).Where("id = ?", id).Delete(&model.ProviderConfig{}).Error
}

func (r *models) getByModelID(ctx context.Context, userID uuid.UUID, id uuid.UUID) (*model.LLM, error) {
	var llm model.LLM
	err := r.db.WithContext(ctx).Preload("ProviderConfig").Where("user_id = ? AND id = ?", userID, id).First(&llm).Error
	if err != nil {
		return nil, err
	}
	return &llm, nil
...
func (r *models) update(ctx context.Context, llm *model.LLM) error {
	return r.db.WithContext(ctx).Save(llm).Error
}

func (r *models) delete(ctx context.Context, userID uuid.UUID, id uuid.UUID) error {
	return r.db.WithContext(ctx).Where("user_id = ? AND id = ?", userID, id).Delete(&model.LLM{}).Error
}
