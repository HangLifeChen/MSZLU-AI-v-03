...
func (h *Handler) GetDocumentContent(c *gin.Context) {
	...
	resp, err := h.service.getDocumentContent(c.Request.Context(), userId, kbId, documentId)
	if err != nil {
		res.Error(c, err)
		return
	}
...
func (h *Handler) ReindexDocument(c *gin.Context) {
	...
	resp, err := h.service.reindexDocument(c.Request.Context(), userId, kbId, documentId)
	if err != nil {
		res.Error(c, err)
		return
	}
