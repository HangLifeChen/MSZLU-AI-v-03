...
func (s *service) uploadDocuments(ctx context.Context, userId uuid.UUID, kbId uuid.UUID, uploadFile *multipart.FileHeader) (any, error) {
	...
	//读取文件内容
	// loader, err := url.NewLoader(
	// 	ctx, &url.LoaderConfig{
	// 		Parser: selectParser,
	// 	},
	// )
...
func (s *service) getDocumentContent(ctx context.Context, userId uuid.UUID, kbId uuid.UUID, documentId uuid.UUID) (*DocumentContent, error) {
	//验证知识库是否存在
	kb, err := s.repo.getKnowledgeBase(ctx, userId, kbId)
	if err != nil {
		logs.Errorf("get knowledge base error: %v", err)
	...
	//统计文档的分块数量
	chunkCount, err := s.repo.countDocumentChunks(ctx, documentId)
	if err != nil {
		logs.Errorf("count document chunks error: %v", err)
		return nil, errs.DBError
	}
	//已处理数量，这里我们用状态为 embedded 的数量作为已处理数量
	//暂时用 chunkCount 作为 processedCount，因为只有成功的才会被存入
	...
	return &DocumentContent{
		...
		KbId:           kbId,
		Name:           doc.Name,
		Description:    "", // Document 模型中没有 description 字段
		FileType:       doc.FileType,
		FileSize:       doc.Size,
		Status:         doc.Status,
		EmbeddingModel: kb.EmbeddingModelName,
...
func (s *service) reindexDocument(ctx context.Context, userId uuid.UUID, kbId uuid.UUID, documentId uuid.UUID) (*DocumentContent, error) {
	//验证知识库是否存在
	kb, err := s.repo.getKnowledgeBase(ctx, userId, kbId)
	if err != nil {
		logs.Errorf("get knowledge base error: %v", err)
	...
	err = s.repo.transaction(ctx, func(tx *gorm.DB) error {
		...
		//删除es的索引
		if kb.StorageType == model.StorageTypeElasticSearch {
			err = s.deleteEsIndex(ctx, kbId, documentId)
			if err != nil {
				logs.Errorf("delete es index error: %v", err)
				return err
		...
		if kb.StorageType == model.StorageTypeMilvus {
			err = s.deleteMilvusIndex(ctx, kbId, documentId)
			if err != nil {
				logs.Errorf("delete milvus index error: %v", err)
				return err
			}
		}
		return nil
	...
	if err != nil {
		logs.Errorf("delete document index error: %v", err)
		return nil, errs.DBError
	}

	//更新状态为处理中
	err = s.repo.updateDocumentStatus(ctx, documentId, model.DocumentStatusProcessing)
	if err != nil {
		logs.Errorf("update document status error: %v", err)
		return nil, errs.DBError
	}
	...
	go func() {
		ctx = context.Background()
		//重新处理文档
		err := s.reprocessDocument(ctx, doc, kb)
		if err != nil {
			logs.Errorf("reprocess document error: %v", err)
			//更新状态为失败
			err = s.repo.updateDocumentStatus(ctx, documentId, model.DocumentStatusFailed)
			if err != nil {
				logs.Errorf("update document status error: %v", err)
				return
			}
		...
		//最后更新状态
		err = s.repo.updateDocumentStatus(ctx, documentId, model.DocumentStatusCompleted)
		if err != nil {
			logs.Errorf("update document status error: %v", err)
			return
		}
...
func (s *service) getChatModel(modelName string, modelProvider string) (aiModel.ToolCallingChatModel, error) {
	...
	switch chatProviderConfig.Provider {
